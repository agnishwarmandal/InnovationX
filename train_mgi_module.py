"""
Script to train the Meta-Gradient Introspection (MGI) module.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("train_mgi.log"),
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train MGI Module")
...
def load_models(args):
    ...
    logger.info(f"Loading models from {args.models_dir} and {args.asp_dir}...")
    ...
    trader_agent.load(os.path.join(args.asp_dir, "trader_agent.pt"))
    adversary_agent.load(os.path.join(args.asp_dir, "adversary_agent.pt"))
...
def train_mgi_module(args, mg, tqe, sp3, trader_agent, adversary_agent, mgi, train_loader):
    """Train the MGI module."""
    logger.info(f"Training MGI module for {args.num_iterations} iterations...")
    ...
    # Create progress bar
    progress_bar = tqdm(total=args.num_iterations, desc="Training MGI")
    ...
    for iteration in range(args.num_iterations):
        ...
        progress_bar.set_postfix({
            "meta_loss": f"{mean_meta_loss.item():.4f}",
            "lr": f"{updated_hyperparams.get('learning_rate', 0):.6f}",
            "risk": f"{updated_hyperparams.get('risk_aversion', 0):.4f}",
        })
        
        # Log progress
        if (iteration + 1) % 10 == 0:
            logger.info(f"Iteration {iteration+1}/{args.num_iterations}, "
                       f"Meta Loss: {mean_meta_loss.item():.4f}")
...
def main():
    ...
    # Load models
    mg, tqe, sp3, trader_agent, adversary_agent = load_models(args)
    ...
    # Load existing MGI module if available
    mgi_path = os.path.join(args.mgi_dir, "mgi_module.pt")
    if os.path.exists(mgi_path):
        mgi.load(mgi_path)
    ...
    if args.profile_performance:
        ...
        with profiler.profile_model(
            model=lambda x: mgi.meta_step(x, None),
            inputs=torch.tensor(1.0, requires_grad=True, device=config.hardware.device),
            trace_filename="profiles/mgi_profile",
        ):
            pass
    
    # Train MGI module
    mgi, meta_metrics = train_mgi_module(
        args, mg, tqe, sp3, trader_agent, adversary_agent, mgi, train_loader
    )
