"""
Script to train the Bayesian Online Mixture (BOM) module.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("train_bom.log"),
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train BOM Module")
...
def train_bom_module(args, mg, tqe, sp3, trader_agent, adversary_agent, bom, train_loader):
    """Train the BOM module."""
    logger.info(f"Training BOM module for {args.num_iterations} iterations...")
    ...
    # Create progress bar
    progress_bar = tqdm(total=args.num_iterations, desc="Training BOM")
    ...
    for iteration in range(args.num_iterations):
        ...
        progress_bar.set_postfix({
            ...
            "reward": f"{metrics['rewards'][-1]:.4f}",
            "uncertainty": f"{metrics['uncertainties'][-1]:.4f}",
        })
        
        # Log progress
        if (iteration + 1) % 10 == 0:
            logger.info(f"Iteration {iteration+1}/{args.num_iterations}, "
                       f"Loss: {metrics['losses'][-1]:.4f}, "
                       f"Reward: {metrics['rewards'][-1]:.4f}, "
                       f"Uncertainty: {metrics['uncertainties'][-1]:.4f}")
...
def test_uncertainty_estimation(args, mg, tqe, sp3, trader_agent, adversary_agent, bom):
...
def main():
    ...
    if args.profile_performance:
        ...
        with profiler.profile_model(
            model=lambda x: bom.predict(x),
            inputs=torch.randn(args.batch_size, input_dim, device=config.hardware.device),
            trace_filename="profiles/bom_profile",
        ):
            pass
    
    # Train BOM module
    bom, metrics = train_bom_module(
        args, mg, tqe, sp3, trader_agent, adversary_agent, bom, train_loader
    )
